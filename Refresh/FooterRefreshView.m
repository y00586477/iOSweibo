...
+ (instancetype)addFooterRefreshViewWithTarget:(id)target action:(SEL)action{
    FooterRefreshView *refreash = [[self alloc] init];
    refreash.hidden = YES;
...
- (void)adjustRefreshView {
    ...
    CGFloat height1 = self.superview.contentSize.height;
    CGFloat y = self.superview.contentOffset.y;
    if (self.superview.isDragging) { //正在拖动
        if ( height + y > height1 && height + y < height1 + headerRefreshHeight && self.currentState == ZCStatePull) { //正常状态->下拉
            self.currentState = ZCStateNormal;
        }else if (height + y >= height1 + headerRefreshHeight && self.currentState == ZCStateNormal)//下拉->正常
        {
            self.currentState = ZCStatePull;
        }
    }else if(self.currentState == ZCStatePull && height + y >= height1 + headerRefreshHeight) { //手释放
...
- (void)setCurrentState:(int)currentState{
    ...
    if (_currentState == ZCStateNormal) {//默认状态
        ...
        [UIView animateWithDuration:0.25 animations:^{
            self.superview.contentInset = UIEdgeInsetsMake(self.superview.contentInset.top, self.superview.contentInset.left, self.superview.contentInset.bottom, self.superview.contentInset.right);
...
- (void)endFooterRefresh {
    if (self.currentState == ZCStateRefresh) {
        self.currentState = ZCStateNormal;
//        [UIView animateWithDuration:0.25 animations:^{
//            self.superview.contentInset = UIEdgeInsetsMake(self.superview.contentInset.top, self.superview.contentInset.left, self.superview.contentInset.bottom - headerRefreshHeight, self.superview.contentInset.right);
//        }];
    }
}
